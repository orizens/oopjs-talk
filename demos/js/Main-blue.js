...
function initBirdsCreationButtons(){
	
	var btns = document.getElementById("birds-btns");
	btns.onmousedown = function(ev) {
		if (ev.srcElement.value) {
			createAngryBird(mouse.x, mouse.y, ev.srcElement.value);
		} else {
...
function reset() {
	...
	for( i = 0; i < numberOfAngryBird; i++ ) {

		createAngryBird(false, false, "red");

	}

	for( i = 0; i < numberOfAngryBird; i++ ) {

		createAngryBird(false, false, "blue");
...
function createAngryBird( x, y, color, isSmall ) {
	...
	element.style['left'] = -200 + 'px';
	element.style['top'] = -200 + 'px';
	createAngryBird.counter++;
	element.id = "bird-" + createAngryBird.counter;
	element.isSmall = isSmall;
	element.color = color;
	...
	for (var i = 2; i > 0; i--) {
		...
		//- MY CODE
		loadAngryBirdIcon(color || "red", element, graphics, isSmall);
	}
	...
	b2body.position.Set( x + 40, y + 40 );
	b2body.linearVelocity.Set( Math.random() * 400 - 200, Math.random() * 400 - 200 );
	bodies.push( world.CreateBody(b2body) );
}
createAngryBird.counter = 0;
function loadAngryBirdIcon(color, oCanvas, context, isSmall) {
	var img = new Image();
	img.onload = function(){
		var width = img.width * 0.1,
...
var Mouse = {
	onRelease: function() {
		if (!isMouseDown) {
			...
			if (mouseJoint) {
				...
				mouseJoint = null;
				if (oEvMouse) {
					console.log(oEvMouse)
					if (oEvMouse.srcElement.color == "blue" && !oEvMouse.srcElement.isSmall){
						var bird = oEvMouse.srcElement;
						setTimeout(function(){
							createAngryBird(mouse.x + 200, mouse.y - 100, "blue", true);
	...
	onPress: function() {
		if (createMode) {
			//- creates many birds on click on canvas
			// createAngryBird( mouse.x, mouse.y );
